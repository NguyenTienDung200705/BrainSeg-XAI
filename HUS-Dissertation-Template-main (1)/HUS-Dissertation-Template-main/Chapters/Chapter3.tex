...
\begin{enumerate}
    ...
    \item \textbf{Kiểm tra kích thước}: Đọc toàn bộ bytes vào RAM (\texttt{raw = await file.read()}), nếu \texttt{len(raw) > 20\,MB} trả về HTTP 413 (Payload Too Large).
    \item \textbf{Kiểm tra tính hợp lệ của ảnh}: Pillow thử mở bytes; nếu không phải ảnh hợp lệ bắt ngoại lệ và trả về HTTP 422.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Giải mã và chuẩn hóa không gian màu}: Bytes được đọc bởi Pillow (\texttt{Image.open(BytesIO(raw))}) và chuyển về RGB. Bước này đảm bảo ảnh grayscale hay RGBA đều được chuẩn hóa về 3 kênh.
    ...
    \item \textbf{Tensor hóa và thêm chiều batch}: \texttt{np.transpose} từ HWC sang CHW, \texttt{torch.FloatTensor}, \texttt{.unsqueeze(0)} để tạo tensor $[1, 3, 256, 256]$.
\end{enumerate}
...
Hàm \texttt{evaluate\_risk(features)} trả về dataclass \texttt{RuleResult} gồm: \texttt{risk\_score}, \texttt{risk\_level} (Thấp/Trung bình/Cao/Rất cao), \texttt{severity}, \texttt{risk\_color} (mã hex), \texttt{fired\_rules} (list quy tắc kích hoạt), \texttt{recommendations} (list khuyến nghị lâm sàng) và \texttt{explanation} (tóm tắt nguyên nhân).
...
\begin{itemize}
    ...
    \item \texttt{ImageViewer.jsx}: Hiển thị năm ảnh base64 qua thẻ \texttt{<img src="data:image/png;base64,...">}, cho phép chuyển đổi qua tab (original / mask / overlay / heatmap / cam\_overlay).
