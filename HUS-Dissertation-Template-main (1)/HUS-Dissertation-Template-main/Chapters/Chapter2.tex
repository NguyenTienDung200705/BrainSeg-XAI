...
\begin{figure}[h!]
    ...
    \caption{Kiến trúc U-Net: nhánh encoder giảm mẫu (trái) tăng số kênh đặc trưng, nhánh decoder tăng mẫu (phải) khôi phục độ phân giải không gian. Các mũi tên xám ngang biểu thị skip connections nối đặc trưng tương ứng từ encoder sang decoder. Ảnh đầu vào $572\times572$ cho ra segmentation map $388\times388$. (Nguồn: \cite{Ronneberger2015})}
    \label{fig:unet_arch}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{So sánh đồ thị kết nối giữa U-Net (trái) và U-Net++ (phải). U-Net chỉ có skip connections thẳng từ encoder sang decoder tương ứng. U-Net++ bổ sung các nút trung gian $x^{i,j}$ trên mỗi skip connection, tạo thành lưới kết nối dày đặc giúp tổng hợp đặc trưng ở nhiều tỷ lệ không gian. (Nguồn: \cite{Zhou2018})}
    \label{fig:unetpp_compare}
\end{figure}

\subsection{EfficientNet-B4 làm encoder}

EfficientNet \cite{Tan2019} sử dụng chiến lược ``compound scaling'' -- đồng thời tăng chiều rộng, chiều sâu và độ phân giải theo một hệ số $\phi$ -- để tối ưu hiệu quả tham số. EfficientNet-B4 với khoảng 19 triệu tham số cho hiệu suất cao trên ImageNet trong khi tiêu thụ ít tài nguyên tính toán hơn so với ResNet-50 hay VGG-16. Việc sử dụng encoder pretrained trên ImageNet giúp khởi tạo với các đặc trưng hình ảnh phong phú, quan trọng khi dữ liệu y tế thường hạn chế.
...
Các đặc trưng được tính trên vùng khối u lớn nhất (largest region) từ \texttt{skimage.measure.regionprops}:
...
\section{Bộ máy luật y khoa (Rule-based Risk Engine)}

\subsection{Thiết kế tập luật}

Bộ máy luật được thiết kế theo nguyên lý \textit{additive scoring}: mỗi quy tắc cộng một điểm cố định vào điểm rủi ro tổng hợp $S_{\text{risk}}$, giới hạn tối đa 100. Thiết kế này cho phép truy vết nguyên nhân đóng góp vào điểm rủi ro cuối cùng. Sáu quy tắc hiện có:
